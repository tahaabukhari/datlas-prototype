...
const MainCenter: React.FC = () => {
    ...
    const { openLeftSidebar, openRightSidebar, isDashboardOpen, openDashboard } = useUI();
    ...
    useEffect(() => {
        try {
            ...
            chat.current = ai.chats.create({
                ...
                config: {
                    systemInstruction: `You are a data analysis assistant named DATLAS.
1. When asked to create a plot or visualization, you MUST respond with only a JSON object.
2. The JSON object must have two keys: "description" (a user-friendly explanation of the plot) and "python_code" (the python code to generate the plot).
3. The python code will be executed in a Pyodide environment with 'pandas' and 'matplotlib' installed.
4. The user's CSV data is pre-loaded into a pandas DataFrame named 'df'. You do not need to load the data.
5. Your python code MUST NOT define or read from a file. Use the 'df' DataFrame directly.
6. Your python code MUST save the plot to an in-memory buffer and output a base64 encoded string of the PNG image. Do not show the plot.
7. Example python code structure:
import matplotlib.pyplot as plt
import io
import base64
# --- Your plotting code using 'df' ---
...
buf.seek(0)
# The final line of output must be the base64 encoded string.
base64.b64encode(buf.read()).decode('utf-8')
8. For all other conversational questions, respond as a helpful assistant with concise, conversational text. DO NOT use JSON for non-plotting responses.`,
                },
            });
        } catch (e) {
    ...
    return (
        <div className="flex-1 flex flex-col bg-zinc-985 h-full relative">
             <header className="md:hidden flex items-center justify-between p-2 border-b border-zinc-800 bg-zinc-985/80 backdrop-blur-sm sticky top-0 z-20">
                <button onClick={openLeftSidebar} className="p-2 text-gray-300 hover:text-white" aria-label="Open menu">
                    <Bars3Icon className="w-6 h-6" />
                </button>
                <h1 className="text-lg font-semibold tracking-wider">DATLAS</h1>
                <button onClick={openRightSidebar} className="p-2 text-gray-300 hover:text-white" aria-label="Open dashboard">
                    <ChartBarIcon className="w-6 h-6" />
                </button>
            </header>

            <div className={`absolute inset-0 items-center justify-center text-center transition-all duration-300 ease-out pointer-events-none ${messages.length > 0 || isLoading ? 'opacity-0 scale-95' : 'opacity-100'} hidden md:flex md:flex-col`}>
                <h1 className="text-4xl font-semibold tracking-wider text-gray-100 animate-subtle-pulse">DATLAS</h1>
            ...
            <div className={`absolute bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 w-full px-3 sm:px-4 z-10 transition-all duration-300 ease-in-out ${isDashboardOpen ? 'max-w-full sm:max-w-lg' : 'max-w-full sm:max-w-2xl'}`}>
                {recorderError && (
                    ...
                    <div className="p-3">
                        ...
                        <div className="relative flex items-center gap-2">
                            ...
                            <button
                                ...
                                className="flex items-center justify-center h-11 w-11 rounded-full shrink-0 transition-colors duration-200 text-gray-400 hover:bg-zinc-800 hover:text-gray-100"
                            ...
                            <div className="relative">
                                <button 
                                    ...
                                    className={`relative z-10 flex items-center justify-center h-11 w-11 rounded-full shrink-0 transition-all duration-200 active:scale-95 ${isRecording ? 'bg-red-900/20 text-red-400' : 'bg-zinc-800 text-gray-100 hover:bg-zinc-700'} disabled:opacity-50 disabled:cursor-not-allowed`}
                                    aria-label={isRecording ? 'Stop recording' : 'Start recording'}
                                    disabled={isLoading || !!recorderError}
                                >
                                    {isRecording ? <StopIcon className="w-5 h-5" /> : <MicrophoneIcon className="w-6 h-6" />}
                                </button>
                                {isRecording && (
                                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                        <div className="absolute h-11 w-11 rounded-full border border-red-500/50 animate-wave-pulse" />
                                        <div className="absolute h-11 w-11 rounded-full border border-red-500/50 animate-wave-pulse [animation-delay:500ms]" />
                                        <div className="absolute h-11 w-11 rounded-full border border-red-500/50 animate-wave-pulse [animation-delay:1000ms]" />
                                    </div>
                                )}
                            </div>
                            <button
                                ...
                                className="flex items-center justify-center h-11 w-11 rounded-full shrink-0 transition-all duration-200 bg-zinc-800 text-gray-100 hover:bg-zinc-700 disabled:opacity-50 disabled:cursor-not-allowed active:scale-95"
